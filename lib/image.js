...
function images(md, opts) {
  const { lazyload, prependRoot, postAsset } = opts;
  const { relative_link, model, base_dir, source_dir } = opts.hexo;
  const hexo = opts.hexo;

  md.renderer.rules.image = function (tokens, idx, options, env, self) {
    const token = tokens[idx];
    const { path: postPath } = env;
      ...
      if (!(src.startsWith("/") || src.startsWith("\\")) && postAsset) {
        ...
        const asset = [
          ...
          join(
            assetDirBasePath,
            src.replace(
              new RegExp("^" + basename(postPath, extname(postPath)) + "/"),
              ""
            )
          ),
        ]
