 ...
 */
function checkPlugins(pugs, hexo) {
  var def_pugs_obj = {};
  ...
  for (var i = 0; i < pugs.length; i++) {
    if (!(pugs[i] instanceof Object) || !(pugs[i].plugin instanceof Object))
      continue;
    var pug_name = pugs[i].plugin.name;
    if (!pug_name) continue;
    if (
      pugs[i].plugin.enable == null ||
      pugs[i].plugin.enable == undefined ||
      pugs[i].plugin.enable != true
    )
      pugs[i].plugin.enable = false;
    if (def_pugs_obj[pug_name]) {
      def_pugs_obj[pug_name] = pugs[i].plugin;
    } else {
      pugs[i].plugin.options = { ...pugs[i].plugin.options, hexo };
      _t.push(pugs[i].plugin);
    }
  }
  ...
  render(data, options) {
    ...
    md = plugins.reduce(function (md, pugs) {
      if (pugs.enable) {
        ...
        } else if (Object.keys(local_pugs).includes(pugs.name)) {
          return md.use(local_pugs[pugs.name], pugs.options);
        } else {
